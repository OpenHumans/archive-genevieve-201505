    ...
    $.fn.filterTable = function(options) { // define the filterTable plugin
        ...
        var doFiltering = function(table, q) { // handle the actual table filtering
                ...
                if (q==='') { // if the filtering query is blank
                    ...
                    //TODO: Connect rows so that relevant information is shared between highlighted row
                    ...
                    var matchedCols = tbody.find('td').removeClass(settings.highlightClass).filter(':filterTableFind("'+q.replace(/(['"])/g,'\\$1')+'")');
                    for (var i=0; i<matchedCols.length; i++){
                      var matchedRow = matchedCols[i].closest('tr');
                      var matchedRowClass = matchedCols[i].prevObject('class');
                      console.log(matchedRowClass)
                    }
                    //.addClass(settings.highlightClass)
                    //.closest('tr').show().addClass(settings.visibleClass)
                    //.siblings('.genome-variant-1332').show().addClass(settings.visibleClass); // highlight (class=alt) only the cells that match the query and show their rows
                }
